...
- `index.html` : 마크다운 파일을 불러와 화면에 그려주는 뷰어. 클레이(Clay) 디자인 톤(크림 배경, 다크 네이비 포인트, 딥티일 코드 박스)을 적용했다.
- `lessons.json` : 이 폴더에서 보여줄 교안 목록. 파일을 추가하면 상단에 탭이 자동으로 생긴다.
- `vendor-marked.min.js`, `vendor-dompurify.min.js` : 마크다운 렌더링에 쓰는 라이브러리. 외부 CDN 없이 내장되어 있어 인터넷 연결이 불안정한 강의실에서도 그대로 열린다.
