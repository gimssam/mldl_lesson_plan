...
# ⚠️ 원본 소스코드는 학습(fit)까지만 실행하며, evaluate()나 정확도 출력 코드는 없음
...
```
Epoch 1/10~10/10  accuracy 1.0000  loss 0.0000e+00
```
※ 원본 코드는 model.fit()까지만 실행하며 evaluate()나 정확도 출력 코드가 없으므로, 위 학습 로그(Keras가 기본 verbose 설정으로 출력하는 epoch별 loss/accuracy) 외에 별도의 "테스트 정확도" 출력은 나타나지 않습니다.

### ③ 자율 주행 차량 — 객체 인식
자율 주행 차량은 카메라, 라이다(LiDAR) 등의 센서를 통해 주변 사물을 인식합니다. <br> 
여기서는 classification23과 동일한 CNN 구조를 CIFAR-10(automobile, truck 등 실제 10개 클래스 중 일부를 포함하는 범용 이미지셋)에
적용하여, 동일한 모델을 자율주행과 같은 다른 응용 맥락으로 설명할 수 있음을 보여주는 예시입니다. <br>
단, CIFAR-10에는 "도로"나 "보행자" 클래스가 없고 차량 관련 클래스도 automobile/truck뿐이므로, 실제 자율주행 인식에는
KITTI, Cityscapes, BDD100K 같은 전용 데이터셋이 사용됩니다.
...
from tensorflow.keras.layers import Dense, Flatten, Conv2D, MaxPooling2D
from tensorflow.keras.utils import to_categorical
(x_train, y_train), (x_test, y_test) = cifar10.load_data()  # automobile, truck 등 CIFAR-10의 실제 10개 클래스 중 일부
x_train, x_test = x_train / 255.0, x_test / 255.0
y_train, y_test = to_categorical(y_train, 10), to_categorical(y_test, 10)
model = Sequential([
    Conv2D(32, (3,3), activation='relu', input_shape=(32,32,3)),
...
- **CIFAR-10 데이터셋 로드**: cifar10.load_data()를 사용하여 automobile, truck 등 CIFAR-10의 실제 10개 클래스 중
일부가 포함된 이미지를 로드합니다. (CIFAR-10에는 "도로"나 "보행자" 클래스는 없습니다.)
- **데이터 전처리**: 이미지를 [0, 255] 범위에서 [0, 1] 범위로 정규화하고, 레이블을 one-hot 인코딩합니다.
...
- **공통 교훈**: 합성/더미 데이터로 실행한 예제들(훈련·평가 정확도를 산출하는 분류 모델 예제 기준)은 대체로 훈련 정확도만 오르고 테스트 정확도는 낮게 나온다(과적합). 단, classification28(gTTS 음성 합성)처럼 훈련/평가 정확도 개념 자체가 없는 API 호출 예제는 이 관찰에서 제외된다. 실행 결과를 통해 실제 데이터의 중요성을 확인할 수 있다.
