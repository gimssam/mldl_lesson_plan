...
### ③ 자율 주행 차량 — 객체 인식
자율 주행 차량은 카메라, 라이다(LiDAR) 등의 센서를 통해 도로와 장애물을 인식합니다. <br> 여기서는 간단한 이미지 분
류 모델을 사용하여 도로에 있는 장애물 객체를 인식하는 예시를 보여줍니다.
...
> *classification23과 동일한 원리로, 합성 데이터를 사용했을 때 훈련 정확도(63%)와 테스트 정확도(12%) 사이의 격차가 오히려 더 크게 벌어졌다. 
<br>모델 구조가 같더라도 데이터와 학습 결과에 따라 과적합의 정도가 달라질 수 있다.<br> 자율 주행처럼 실제 도로 위 안전이 걸린 문제에서는 이런 과적합을 방치하면 위험할 수 있다.*
...
> *네 문장짜리 초소형 더미 데이터다. 초반 정확도(0.250)가 낮고 손실도 크게 줄지 않지만, 학습이 진행되며 정확도가 0.750까지 오르는 흐름은 확인할 수 있다. <br>학습에 쓰이지 않은 새 문장 "I love this film"에 대해서도 임베딩과 LSTM을 거쳐 "긍정"으로 올바르게 예측한 점이 핵심이다.<br> 실제 서비스에서는 수만~수십만 문장 규모의 데이터로 학습해야 이런 패턴을 안정적으로 일반화할 수 있다.*
